 ...
 *
 * 修改了地图填充颜色的方式，由外部配置图块颜色修改为在绘制图块的同时为图块填充颜色
 * 新增了文字显示功能，
 * 新增了设置地图边框颜色的功能
 * 新增了设置地图边框宽度的功能
 * 新增返回按钮功能
...
(function( $ ){
    ...
    $.fn.vectorMap = function(options) {
        var defaultParams = {
			...
			borderColor:'#ffffff',//地图的边框线颜色
			borderWidth:'1',//地图的边框线宽度
	...
	VectorCanvas.prototype = {
        ...
        createPath: function(config) {
            ...
            if (this.mode == 'svg') {
				...
				//新增，添加地图边框线颜色。chenting添加于2013-06-19
				node.setStroke = function(borderColor) {
					if(undefined != borderColor && "" != borderColor){
						this.setAttribute("stroke",borderColor);
					}
				};
				//新增，添加地图边框线宽度。chenting添加于2013-06-19
				node.setStrokeWidth = function(borderWidth) {
					if(undefined != borderWidth && "" != borderWidth){
						this.setAttribute("stroke-width",borderWidth);
					}
				};
    ...
    var WorldMap = function(params) {
		...
		this.borderColor = params.borderColor;//每个地图的边框线颜色
		this.borderWidth = params.borderWidth;//每个地图的边框线宽度
		...
		//返回按钮
		$('<div/>').addClass('jvectormap-zoompic').html('<img src="images/btn1.png">').appendTo(params.container);
		$(".jvectormap-zoompic").click(function(){
			history.go(-1);
		});
		...
		for(var key in mapData.pathes) {
			...
			//自定义填充颜色。chenting修改于2013-06-19
			var fillColor = mapData.pathes[key].color;
			if(undefined != fillColor && "" != fillColor){
			...
			//为每个地图添加边框颜色和边框宽度
			var color = mapData.pathes[key].borderColor;
			var width = mapData.pathes[key].borderWidth;
			if(undefined != color && "" != color){
				path.setStroke(color);
			}else{
				path.setStroke(this.borderColor);
			}
			if(undefined != width && "" != width){
				path.setStrokeWidth(width);
			}else{
				path.setStrokeWidth(this.borderWidth);
			}			
		...
		$(params.container).delegate(this.canvas.mode == 'svg' ? 'path' : 'shape', 'mouseover mouseout click', function(e){
			...
			if (e.type == 'mouseover') {
				...
				var names = mapData.pathes[code].name;
				map.label.html(names);
				$(params.container).trigger(labelShowEvent, [map.label, code]);
				if (!labelShowEvent.isDefaultPrevented()) {
					/*if(undefined != names && "" != names){
						map.label.show();
					}*/
					map.label.show();
	...
	WorldMap.prototype = {
		...
		setBackgroundImg: function(backgroundImg) {
			this.container.css('background-image',backgroundImg);
			this.container.css('background-repeat','no-repeat');
		},
